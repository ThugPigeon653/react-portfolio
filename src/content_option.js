...
const dataportfolio = [
    {
        img: "images/testQL.png",
        title: "TestQL",
        description: "A tool for automating SQL unit testing",
        details:<p>TestQL is an open source project I developed, which abstracts away the complexity of automating CI pipelines for SQL. The documentation and linked demo video can be found in the repository (README.md), and are the best way to get an understanding of this program. <br/>The tool works by setting up directories and generating files on-demand, meaning an entire CI workflow can be created with just one command. in addition to the workflow, it generates a unit test file which is already correctly setup to handle connections to the automatically-provisioned database container. This makes unit testing of SQL files in GitHub repositories a trivial task.<br/><br/>On a technical level, this tool was inspired by the React framework, in that it dynamically writes and saves code as required. Code fragments are stored in a json config file, and accessed by their keys. This approach makes adding new functionality to the tool extremely agile, because this way the only file that ever needs alterations is the config file - the Python logic can remain untouched. To add a new testing pipeline, I simply add a new engine key and relevant config data, and thats it.<br/>For convenience of usage, I have provided a build script, which creates an exe from source code, and adds the program to PATH. This way, users have access to the CLI in any terminal window on that machine.<br/> When it comes time to make a push to the repository, your SQL unit testing CI will be run before changes are actually made to the current branch.<br/><br/>In addition to being CI software itself, the repository contains a CI implementation of it's own. Because this is an open source repository, I have used GitHub Actions to ensure nobody can push changes to the main branch, forcing other developers to work on their own branch, and make a pull request when it comes time. I have also protected the main branch, and included a rule in the workflow file which prevents anyone except for myself from making changes to the workflow itself. This will protect against malicious interferance, but because this is open source the main reason for these rules is really to protect against accidental interferance (i.e. a developer accidentally altering the workflow).<br/><br/>The repository is actually linked with another repo of mine (sql-multilanguage), which acts as a set of example implementations of this tool. It displays how a user might go about testing multiple SQL files on different engines, all in the same repository. I have also made sure to use procedural elements in each language, to prove that the pipelines do work for procedural SQL.</p>,
        link: "https://github.com/ThugPigeon653/testQL-source",
    },
    {
